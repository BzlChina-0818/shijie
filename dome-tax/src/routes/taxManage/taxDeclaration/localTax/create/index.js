...
const Create = ({
  ...
  // state or variable
  const {formData, pageType, modalVisible, tableProps, taxPayer = "", taxPayerNo = "", NormTypeList,} = localTaxCreate
  ...
  const hideModal = () => {
    dispatch({
      type: "localTaxCreate/updateState",
      payload: {
        modalVisible: ""
      }
    });
...
export default connect(({localTaxCreate, loading}) => ({
  localTaxCreate,
  loading
}))(Form.create()(Create))
